...
func main() {
	...
	// Check now much is stored in buffer waiting
	unflusedBufferSize := bufferedWriter.Buffered()
	...
	// See now much buffer is available
	bytesAvailable = bufferedWriter.Available()
	if err != nil {
		log.Fatal(err)
	}
	...
	// check available buffer size after resizeng
	bytesAvailable = bufferedWriter.Available()
	if err != nil {
		log.Fatal(err)
	}
